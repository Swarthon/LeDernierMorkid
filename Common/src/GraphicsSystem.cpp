...
namespace Common {
	...
	void GraphicsSystem::initialize( const Ogre::String &windowTitle ) {
		...
		mRoot->getRenderSystem()->setConfigOption("Video Mode", "1920 x 1080 @ 32-bit colour");
	...
	void GraphicsSystem::chooseSceneManager(void) {
		...
		mSceneManager->setAmbientLight( Ogre::ColourValue(1.f,1.f,1.f), Ogre::ColourValue(0.f,0.f,0.f), Ogre::Vector3(0,1,0) );
	}
	//-----------------------------------------------------------------------------------

	...
	namespace Mq {
		class MessageQueueSystem {
			...
			static void storeMessageToQueue(MessageArray& queue,
				...
				const size_t totalSize = Ogre::alignToNextMultiple(
				        cSizeOfHeader + sizeof(T), sizeof(size_t));
		...
		protected:
			void processIncomingMessages(void) {
				...
				while (itor != end) {
					...
					Ogre::uint32 messageId = *reinterpret_cast<const Ogre::uint32*>(
					        itor + sizeof(Ogre::uint32));
					...
					processIncomingMessage(
					        static_cast<Mq::MessageId>(messageId), data);
					itor += totalSize;
				}

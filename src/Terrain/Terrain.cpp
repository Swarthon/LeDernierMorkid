...
Terrain::Terrain( Ogre::IdType id, Ogre::ObjectMemoryManager *objectMemoryManager,
...
//-------------------------------------------------------------------------------------------------
void Terrain::build(const Ogre::String &texName, const Ogre::Vector3 center, const Ogre::Vector3 &dimensions){
	mTerrainOrigin = center - dimensions * 0.5f;
		...
		mXZRelativeSize = mXZDimensions / Ogre::Vector2( static_cast<Ogre::Real>(mWidth),static_cast<Ogre::Real>(mDepth) );
...
void Terrain::createHeightmapTexture( const Ogre::String &imageName, const Ogre::Image &image ) {
	...
	mHeightMapTex = Ogre::TextureManager::getSingleton().createManual(
				...
				Ogre::TEX_TYPE_2D, (uint)image.getWidth(), (uint)image.getHeight(),
				numMipmaps, image.getFormat(), Ogre::TU_STATIC_WRITE_ONLY );

	for( Ogre::uint8 mip=0; mip<=numMipmaps; ++mip ) {
		Ogre::v1::HardwarePixelBufferSharedPtr pixelBufferBuf = mHeightMapTex->getBuffer(0, mip);
		const Ogre::PixelBox &currImage = pixelBufferBuf->lock( Ogre::Box( 0, 0,
...
//-------------------------------------------------------------------------------------------------
void Terrain::addRenderable(const GridPoint &gridPos, const GridPoint &cellSize, Ogre::uint32 lodLevel) {
...
bool Terrain::isVisible( const GridPoint &gPos, const GridPoint &gSize ) const {
	...
	const Ogre::Vector2 cellSize( (gSize.x + 1u) * mXZRelativeSize.x, (gSize.z + 1u) * mXZRelativeSize.y );

...
const IdString* TerrainProperty::DetailOffsetsPtrs[4] = {&TerrainProperty::DetailOffsets0,
...
const HlmsCache* HlmsTerrain::createShaderCacheEntry(uint32                  renderableHash,
	...
	if (mShaderProfile == "hlsl") {
		mListener->shaderCacheEntryCreated(
		        mShaderProfile, retVal, passCache, mSetProperties, queuedRenderable);
	...
	if (!retVal->pso.pixelShader.isNull()) {
		...
		assert(dynamic_cast<const HlmsTerrainDatablock*>(
		...
		for (int i = 0; i < numTextures; ++i)
			psParams->setNamedConstant(
			        "textureMaps[" + StringConverter::toString(i) + "]", texUnit++);
...
void HlmsTerrain::setDetailMapProperties(HlmsTerrainDatablock* datablock, PiecesMap* inOutPieces) {
	...
	for (size_t i = 0; i < 4; ++i) {
		setDetailTextureProperty(
		        TerrainProperty::DetailMapN, datablock, TERRAIN_DETAIL0, i);
		setDetailTextureProperty(
		        TerrainProperty::DetailMapNmN, datablock, TERRAIN_DETAIL0_NM, i);
		setDetailTextureProperty(
		        TerrainProperty::RoughnessMap, datablock, TERRAIN_DETAIL_ROUGHNESS0, i);
		setDetailTextureProperty(
		        TerrainProperty::MetalnessMap, datablock, TERRAIN_DETAIL_METALNESS0, i);
...
HlmsCache HlmsTerrain::preparePassHash(const CompositorShadowNode* shadowNode,
	...
	if (casterPass) {
		HlmsCache retVal = Hlms::preparePassHashBase(
		        shadowNode, casterPass, dualParaboloid, sceneManager);
		return retVal;
	}
	...
	setProperty(TerrainProperty::HwGammaWrite,
	            capabilities->hasCapability(RSC_HW_GAMMA) && renderTarget->isHardwareGammaEnabled());
	setProperty(TerrainProperty::SignedIntTex,
	...
	if (shadowNode)
		...
		mapSize += (3 * 4 * 4) * numDirectionalLights; // Three variables * 4 (padded vec3)
	                                                       // * 4 (bytes) * numDirectionalLights
	...
	if (shadowNode) {
		...
		const CompositorShadowNode::LightsBitSet& affectedLights = shadowNode->getAffectedLightsBitSet();
		...
		for (int32 i = 0; i < numLights; ++i) {
			...
			*passBufferPtr++  = 1.0f / (cosf(innerAngle.valueRadians() * 0.5f) - cosf(outerAngle.valueRadians() * 0.5f));
		...
		for (int32 i = 0; i < numDirectionalLights; ++i) {
			...
			*passBufferPtr++ = lightPos.z;
			...
			// vec3 lights[numLights].diffuse
			ColourValue colour = globalLightList.lights[i]->getDiffuseColour() * globalLightList.lights[i]->getPowerScale();
			*passBufferPtr++   = colour.r;
			*passBufferPtr++   = colour.g;
			*passBufferPtr++   = colour.b;
			++passBufferPtr;
	...
	passBufferPtr = mListener->preparePassBuffer(
	        shadowNode, casterPass, dualParaboloid, sceneManager, passBufferPtr);
...
uint32 HlmsTerrain::fillBuffersFor(const HlmsCache*        cache,
                                   ...
                                   bool                    isV1) {
	assert(
	        dynamic_cast<const HlmsTerrainDatablock*>(queuedRenderable.renderable->getDatablock()));
	...
	if (OGRE_EXTRACT_HLMS_TYPE_FROM_CACHE_HASH(lastCacheHash) != HLMS_USER3) {
		...
		*commandBuffer->addCommand<CbShaderBuffer>() = CbShaderBuffer(
		        VertexShader, 0, passBuffer, 0, passBuffer->getTotalSizeBytes());
		*commandBuffer->addCommand<CbShaderBuffer>() = CbShaderBuffer(
		...
		if (mGridBuffer) {
			texUnit                                      = 5;
			*commandBuffer->addCommand<CbShaderBuffer>() = CbShaderBuffer(PixelShader, 3, mGridBuffer, 0, 0);
			*commandBuffer->addCommand<CbShaderBuffer>() = CbShaderBuffer(PixelShader, 4, mGlobalLightListBuffer, 0, 0);
		}
		...
		while (itor != end) {
			*commandBuffer->addCommand<CbTexture>() = CbTexture(texUnit, true, *itor, mCurrentShadowmapSamplerblock);
			++texUnit;
			++itor;
		}
		...
		if (mCurrentConstBuffer < mConstBuffers.size() && (size_t)((mCurrentMappedConstBuffer - mStartMappedConstBuffer) + 16) <= mCurrentConstBufferSize) {
			*commandBuffer->addCommand<CbShaderBuffer>() = CbShaderBuffer(
	...
	if (mLastBoundPool != datablock->getAssignedPool() && (!casterPass || datablock->getAlphaTest() != CMPF_ALWAYS_PASS)) {
	...
	if (mLastMovableObject != queuedRenderable.movableObject) {
		...
		*commandBuffer->addCommand<CbTexture>() = CbTexture(1, true, terraObj->getNormalMapTex().get(), mTerrainSamplerblock);
		...
		if (itor == end) {
			assert(
			        "Hazard Detected! You should expose this Terrain's shadow map texture"
		...
		if (datablock->mTextureHash != mLastTextureHash) {
			...
			while (itor != end) {
				*commandBuffer->addCommand<CbTexture>() = CbTexture(
				        texUnit++, true, itor->texture.get(), itor->samplerBlock);
				++itor;
			}

...
void TerrainCell::initialize(Ogre::VaoManager* vaoManager) {
	...
	Ogre::VertexArrayObject*    vao = vaoManager->createVertexArrayObject(vertexBuffers, 0, Ogre::OT_TRIANGLE_STRIP);
...
bool TerrainCell::merge(TerrainCell* next) {
	...
	if (mLodLevel == next->mLodLevel) {
		...
		if ((this->mGridX + this->mSizeX == next->mGridX || next->mGridX + next->mSizeX == this->mGridX) && mGridZ == next->mGridZ && mSizeZ == next->mSizeZ) {
			// Merge horizontally
		...
		else if ((this->mGridZ + this->mSizeZ == next->mGridZ || next->mGridZ + next->mSizeZ == this->mGridZ) && mGridX == next->mGridX && mSizeX == next->mSizeX) {
			// Merge vertically
...
//-------------------------------------------------------------------------------------------------
void TerrainCell::setOrigin(const GridPoint& gridPos,
                            Ogre::uint32     horizontalPixelDim,
                            Ogre::uint32     verticalPixelDim,

...
//------------------------------------------------------------------------------------------------
HlmsTerrainDatablock::HlmsTerrainDatablock(IdString              name,
                                           HlmsTerrain*          creator,
...
//------------------------------------------------------------------------------------------------
void HlmsTerrainDatablock::decompileBakedTextures(
        TerraBakedTexture outTextures[NUM_TERRAIN_TEXTURE_TYPES]) {
...
//------------------------------------------------------------------------------------------------
void HlmsTerrainDatablock::bakeTextures(
        const TerraBakedTexture textures[NUM_TERRAIN_TEXTURE_TYPES]) {
	mBakedTextures.clear();

	for (size_t i = 0; i < NUM_TERRAIN_TEXTURE_TYPES; ++i) {
		if (!textures[i].texture.isNull()) {
			TerraBakedTextureArray::const_iterator itor = std::find(mBakedTextures.begin(), mBakedTextures.end(), textures[i]);
...
void HlmsTerrainDatablock::setRoughness(uint8 detailMapIdx, float roughness) {
	...
	if (mRoughness[detailMapIdx] <= 1e-6f) {
		LogManager::getSingleton().logMessage("WARNING: TERRA Datablock '" + mName.getFriendlyText() + "' Very low roughness values can "
...
//------------------------------------------------------------------------------------------------
void HlmsTerrainDatablock::_setTextures(
        const PackedTexture packedTextures[NUM_TERRAIN_TEXTURE_TYPES]) {
...
//------------------------------------------------------------------------------------------------
void HlmsTerrainDatablock::setTexture(TerrainTextureTypes     texType,
                                      uint16                  arrayIndex,
	...
	if (oldTex == textures[texType])
		scheduleConstBufferUpdate(); // Only the array index changed. Just update our
	                                     // constant buffer.
	else
		bakeTextures(textures);
}
...
//------------------------------------------------------------------------------------------------
void HlmsTerrainDatablock::setSamplerblock(TerrainTextureTypes     texType,
